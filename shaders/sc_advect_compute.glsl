...
void advect_lossless() {
    ...
    vec2 rb00 = imageLoad(heightmap_in, src00).rb;
    vec2 rb10 = imageLoad(heightmap_in, src10).rb;
    vec2 rb01 = imageLoad(heightmap_in, src01).rb;
    vec2 rb11 = imageLoad(heightmap_in, src11).rb;
    vec2 rb0 = mix(rb00, rb10, frac_src.x);
    vec2 rb1 = mix(rb01, rb11, frac_src.x);
    vec2 h_src = mix(rb0, rb1, frac_src.y);
    float a_src = h_src.x;
