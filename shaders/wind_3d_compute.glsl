...
layout(set = 1, binding = 0, r32f) uniform readonly image2D heightmap;

layout(std140, set = 1, binding = 1) readonly buffer WindSSBOIn {
    vec4 surf_vec[ ];
};

layout(std140, set = 1, binding = 2) buffer WindSSBOOut {
    vec4 wind_vec[ ];
};
...
void main() {
    ...
    uint idx3d = size * y_layers * pz + y_layers * py + px;

    float y = y_m - 1.0 - float(py);
    float a = imageLoad(heightmap, ivec2(px, pz)).r;
    float y_l = a / (1.5 * MAX_HEIGHT);
    float y_f = floor(y_l - 0.5);
    vec3 w = vec3(0.0);
    vec3 w_max = normalize(vec3(WIND_VEC.x, 0.0, WIND_VEC.y));
